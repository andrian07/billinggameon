...
/// Rekap pengisian saldo member — Report/saldo_report, dengan filter tanggal
/// (wajib), member, dan kasir, serta export ke Excel/PDF.
class SaldoReportPage extends StatefulWidget {
  const SaldoReportPage({super.key});

  @override
  State<SaldoReportPage> createState() => _SaldoReportPageState();
}

class _SaldoReportPageState extends State<SaldoReportPage> {
  ...
  Future<void> _runReport() async {
    ...
    try {
      final result = await _repository.getSaldoReport(
        dateFrom: _dateFrom,
        dateTo: _dateTo,
  ...
  Future<void> _export(String type) async {
    ...
    try {
      final file = await _repository.exportSaldoReport(
        dateFrom: _dateFrom,
        dateTo: _dateTo,
        customerId: _selectedCustomer?.id,
        paidBy: _selectedKasir?.id,
      ...
      final savePath = await FilePicker.platform.saveFile(
        dialogTitle: "Simpan Laporan Saldo",
  ...
  Widget build(BuildContext context) {
    return AppLayout(
      title: "Laporan Saldo",
      subtitle: "Rekap pengisian saldo member",
      showSearch: false,
      activeMenuKey: "laporan_saldo",
  ...
  Widget _buildBody() {
    ...
    if (result == null || result.rows.isEmpty) {
      return _buildMessageState(
        ...
        message: "Tidak ada pengisian saldo pada rentang tanggal ini",
      );
    }
    ...
    return Column(
      children: [
        Padding(
          ...
          child: _SaldoReportRowWidget.header(),
        ),
        Expanded(
          child: ListView.separated(
            ...
            itemBuilder: (context, index) =>
                _SaldoReportRowWidget.data(no: index + 1, row: result.rows[index]),
          ),
        ),
        const Divider(height: 1, color: AppColors.divider),
        Container(
  ...
  Widget _buildSummary(SaldoReportSummary summary) {
    return Row(
      children: [
        _summaryStat("Jumlah Nota", "${summary.invoiceCount}"),
        const SizedBox(width: 28),
        _summaryStat("Total Nominal", formatCurrency(summary.totalNominal)),
        const SizedBox(width: 28),
        _summaryStat("Total Diskon", formatCurrency(summary.totalDiscount)),
        const Spacer(),
        _summaryStat(
          "Total Dibayar",
...
class _SaldoReportRowWidget extends StatelessWidget {
  final bool header;
  final int? no;
  final SaldoReportRow? row;

  const _SaldoReportRowWidget.header() : header = true, no = null, row = null;

  const _SaldoReportRowWidget.data({required this.no, required this.row})
    : header = false;
  ...
  Widget build(BuildContext context) {
    if (header) {
      return _row(
        ...
        dibayar: _headerText("Dibayar", alignEnd: true),
    ...
    return Padding(
      ...
      child: _row(
        ...
        tanggal: Text(formatDate(r.time), style: cellStyle),
